...
import { 
  Trophy, Globe, TrendingUp, Users, Shield, Leaf, DollarSign, Activity, Sparkles, RefreshCw, Landmark, AlertTriangle 
} from 'lucide-react';
...
export default function FifaBoardDashboard() {
  ...
  // Mock revenue data for executive reporting
  const revenueData = [
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between p-6 rounded-2xl bg-gradient-to-r from-amber-950/40 via-slate-900/60 to-slate-900/40 border border-amber-950/30">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="warning" className="px-3 py-1 uppercase tracking-widest text-[10px]">
          ...
          <Button 
            ...
            disabled={refreshing}
            className="text-xs flex gap-1.5 items-center border-slate-800 bg-[#0c101d] text-amber-400 hover:text-amber-300"
          >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400">Global Attendance</CardTitle>
            <Users className="h-4 w-4 text-cyan-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-black text-white">324,500</div>
            <p className="text-[10px] text-emerald-400 mt-1 flex items-center gap-1">
              <TrendingUp className="h-3 w-3" /> +14.2% over target projections
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400">TV & Digital Viewers</CardTitle>
            <Globe className="h-4 w-4 text-amber-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-black text-white">428.5 M</div>
            <p className="text-[10px] text-slate-400 mt-1">Live stream peaks: 24.3 M concurrent</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400">Total Portal Revenue</CardTitle>
            <DollarSign className="h-4 w-4 text-emerald-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-black text-white">$23.68 M</div>
            <p className="text-[10px] text-emerald-400 mt-1 flex items-center gap-1">
              <TrendingUp className="h-3 w-3" /> Ticket margins at all-time high
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400">Tournament Safety Index</CardTitle>
            <Shield className="h-4 w-4 text-purple-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-black text-white">99.8%</div>
            <p className="text-[10px] text-slate-400 mt-1">Response threshold: &lt; 90 seconds</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 flex flex-col h-[400px]">
          ...
          <CardContent className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {stadiumStatusData.map((stadium, idx) => (
              <div key={idx} className="p-3.5 rounded-lg border border-slate-900 bg-slate-950/30 text-xs space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-slate-900/60 pt-2 text-[10px] text-slate-400 font-mono">
                  ...
                  <div>
                    Active Incidents: <span className={stadium.alerts > 0 ? "text-amber-400 font-bold" : "text-white font-bold"}>{stadium.alerts}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 flex flex-col h-[320px]">
          ...
          <CardContent className="flex-1 flex flex-col justify-between py-4">
            ...
            <div className="grid grid-cols-3 text-center text-[10px] text-slate-400 border-t border-slate-900/80 pt-3">
              {sustainabilityData.map((item, idx) => (
                <div key={idx} className="flex flex-col items-center">
                  <span className="font-semibold truncate max-w-[80px]">{item.name}</span>
                  <span className="text-emerald-400 font-black font-mono mt-0.5">{item.value}%</span>
                </div>
              ))}
            </div>
          </CardContent>
