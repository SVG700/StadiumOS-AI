...
import { 
  ...
  RefreshCw, ClipboardList, AlertOctagon, CheckSquare, CheckCircle, Trophy
} from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight text-white">Stadium Operations Command</h2>
      ...
      <Card className="bg-[#080d19]/45 border-slate-900/60 p-3 flex flex-col sm:flex-row items-center justify-between gap-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {/* Active Crowd */}
        <Card className="bg-[#080d19]/45 border-slate-900/60 p-3">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Crowd Count</span>
            <Users className="h-4 w-4 text-blue-400" />
          </div>
          <div className="text-xl font-black text-white">{visitors.total.toLocaleString()}</div>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 p-3">
          ...
          <div className="text-xl font-black text-white">{visitors.staff}</div>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 p-3">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Active Incidents</span>
            <AlertOctagon className="h-4 w-4 text-amber-400" />
          </div>
          <div className="text-xl font-black text-amber-400">{openIncidents.length}</div>
        ...
        <Card className="bg-[#080d19]/45 border-slate-900/60 p-3">
          ...
          <div className="text-xl font-black text-white">{tasks.filter(t => t.status === 'pending').length}</div>
          <p className="text-[10px] text-cyan-400 mt-0.5">{tasks.filter(t => t.priority === 'high' && t.status === 'pending').length} High Priority</p>
        </Card>
      </div>

      {/* MAIN CONSOLE 2-COLUMN GRID */}
      <div className="grid gap-4 md:grid-cols-2">
        {/* Left Column: Live Alerts & Emergency Dispatch + Pending Tasks */}
        <div className="space-y-4">
          {/* Emergency Dispatch */}
          <Card className="bg-[#080d19]/45 border-slate-900/60 flex flex-col h-[230px]">
            ...
            <CardContent className="flex-1 overflow-y-auto px-4 py-2.5 space-y-2">
                ...
                activeAlerts.map((alert) => (
                  <div key={alert.id} className="p-2.5 rounded-lg border border-slate-800 bg-[#080d19]/60 text-xs space-y-1">
                    <div className="flex items-center justify-between font-bold">
                      <span className="text-white truncate max-w-[180px]">{alert.title}</span>
          ...
          {/* Pending Tasks */}
          <Card className="bg-[#080d19]/45 border-slate-900/60 flex flex-col h-[220px]">
            <CardHeader className="py-2.5 px-4 border-b border-slate-900/40">
                ...
                <div key={task.id} className="p-2 rounded-lg border border-slate-800 bg-[#070b13]/40 text-xs">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-semibold text-slate-200 text-[11px] truncate max-w-[180px] ${task.status === 'completed' ? 'line-through text-slate-500' : ''}`}>
                        {task.title}
                      </span>
                    </div>
        ...
        {/* Right Column: Operations Timeline & Open Incidents */}
        <div className="space-y-4">
          {/* Operations Timeline */}
          <Card className="bg-[#080d19]/45 border-slate-900/60 flex flex-col h-[230px]">
            <CardHeader className="py-2.5 px-4 border-b border-slate-900/40">
            ...
            <CardContent className="flex-1 overflow-y-auto px-4 py-2.5 space-y-2.5">
              <div className="relative border-l border-slate-800 ml-1 pl-4 space-y-2.5">
              ...
              {incidents.length === 0 ? (
                ...
                incidents.map((inc) => (
                  <div key={inc.id} className="p-2.5 rounded-lg border border-slate-800/80 bg-[#070b13]/50 text-xs space-y-1">
                    <div className="flex items-center justify-between font-bold">
                      <span className="text-white truncate max-w-[180px]">{inc.issue}</span>
